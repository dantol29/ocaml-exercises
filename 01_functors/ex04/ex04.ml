...
module type MAKEEVALEXPR = functor (X : VAL) -> (EVALEXPR with type t = X.t)

module MakeEvalExpr : MAKEEVALEXPR = functor (X: VAL) -> struct
  type t = X.t
  type expr = Value of t | Add of expr * expr | Mul of expr * expr
  let rec eval ex = match ex with
    | Value n -> n
    | Add (a, b) -> X.add (eval a) (eval b)
    | Mul (a, b) -> X.mul (eval a) (eval b)
end
...
let se = StringEvalExpr.Mul (StringEvalExpr.Value "very ", 
  (StringEvalExpr.Add (StringEvalExpr.Value "very long", StringEvalExpr.Value "short")))
