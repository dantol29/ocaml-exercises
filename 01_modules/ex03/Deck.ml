...
let newDeck () : t = 
  let rec contains l (color, value) =
    match l with
    | [] -> false
    | (c, v) :: tail -> (
      if c = color && v = value then
        true
      else
        contains tail (color, value)
    )
  in

  let rec aux n acc all =
    if n = 0 then acc
    else (
      Random.self_init ();
      let index = Random.int (List.length all) in
      let c = List.nth all index in
      
      if contains acc c then
        aux n acc all
      else
        aux (n - 1) (c :: acc) all
    )
...
let toStringList (d: t) : string list =
  List.map (fun x -> Card.toString x) d

let toStringListVerbose (d: t) : string list =
  List.map (fun x -> Card.toStringVerbose x) d

let drawCard (d: t) : (Card.t * t) = 
  match d with
  | [] -> raise (Failure "no cards in the deck")
  | head :: tail -> (head, tail)